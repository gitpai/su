...
<body>
	...
	<table class="table table-border table-bordered table-bg">
		...
		<c_rt:choose>
					...
					<td class="td-status"><span umbrella-sta="true" id="umbrella-sta" class="label label-success radius" >未借出</span></td></c_rt:when>
					<c_rt:otherwise>
					<td class="td-status"><span umbrella-sta="false"id="umbrella-sta" class="label radius" >借出</span></td></c_rt:otherwise>
		</c_rt:choose>	
		<td>
				 <button align="center" type="button" class="btn btn-success" id="borrow" >借</button>				 
				 &nbsp;&nbsp; 
				 <button align="center" type="button" class="btn btn-danger" id="reback" >还</button>
		</td>
		</tr>
		</c_rt:forEach>
...
jQuery(document).on('click', "#borrow", function() {
	...
	alert(cur.attr("umbrella-sta"));
	$("#umbrella-sta").text("借出");
	alert("${uuid}");
	var data={
   		lock:true,
...
/*管理员-删除*/
function brrow(obj,id){
	layer.confirm('确认要借伞吗？',function(index){
		$.ajax({
			type: 'POST',
			url: 'getDeviceState',
			dataType: 'json',
...
function reback(obj,id){
	layer.confirm('确认要还伞吗？',function(index){
		$.ajax({
			type: 'POST',
			url: 'getDeviceState',
			dataType: 'json',
